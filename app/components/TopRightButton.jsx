...
export default function TopRightButton() {
	...
	useEffect(() => {
		...
		function handleResize() {
			const isMobile = window.innerWidth <= 768; // 判断是否为手机屏幕
			setButtonStyle({
				width: isMobile ? 80 : 100, // 手机屏幕宽度为 80px，桌面为 100px
				height: isMobile ? 28 : 40, // 手机屏幕高度为 32px，桌面为 40px
				top: isMobile ? 20 : 50, // 手机屏幕距离顶部为 20px，桌面为 50px
				right: isMobile ? 20 : 50, // 手机屏幕距离右侧为 20px，桌面为 50px
				fontSize: isMobile ? 18 : 36, // 手机屏幕字体大小为 18pt，桌面为 24pt
				innerFontSize: isMobile ? 10 : 14, // 手机屏幕内文字大小为 14pt，桌面为 20pt
			});
		}

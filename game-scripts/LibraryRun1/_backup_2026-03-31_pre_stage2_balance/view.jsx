...
function Stage3TimeLoopOverlay({ battle }) {
  ...
  const shadow = [...(battle.playerUnits || []), ...(battle.enemyUnits || [])].find((unit) => unit.id === "p-robot" && unit.alive);
  const carter = [...(battle.playerUnits || []), ...(battle.enemyUnits || [])].find((unit) => unit.id === "s3-alchemist-carter" && unit.alive);
  const shadowLoops = shadow?.runtimeState?.stage3TimeLoops || [];
  const carterLoops = carter?.runtimeState?.stage3TimeLoops || [];
  const playerCandidates = battle?.stageRuntime?.stage3TimeLoopCandidates?.player || [];
  const enemyCandidates = battle?.stageRuntime?.stage3TimeLoopCandidates?.enemy || [];
  const leftValues = [0, 1, 2, 3, 4];
  const rightValues = [5, 6, 7, 8, 9];
  const renderGroup = (values) => values.map((value) => (
    <div key={value} className="stage3-loop-slot">
      {enemyCandidates.includes(value) ? <div className="stage3-loop-candidate down">▼</div> : null}
      {carterLoops.includes(value) ? <div className="stage3-loop-arrow down">↓</div> : null}
      <div className="stage3-loop-number">{value}</div>
      {shadowLoops.includes(value) ? <div className="stage3-loop-arrow up">↑</div> : null}
      {playerCandidates.includes(value) ? <div className="stage3-loop-candidate up">▲</div> : null}
    </div>
  ));
...
export default function LibraryRun1View({ gameState, onAction }) {
  ...
  }, [allUnits, battle?.stageId, displayTurn]);
  const stage3UpcomingCoopIds = useMemo(() => {
    ...
    return result;
  }, [allUnits, battle?.stageId, battle?.stageRuntime?.stage3CooperationEnabled, battle?.stageRuntime?.stage3ForceFirstFailedCoop, stage3CurrentAttackerId]);
  ...
  return (
    <style jsx global>{`
      ...
      .stage3-loop-arrow::before {
        ...
        text-shadow:
          0 0 18px rgba(255, 72, 72, 0.4),
          0 0 34px rgba(255, 72, 72, 0.24);
        opacity: 0.88;
      }

      .stage3-loop-arrow.down::before {
        content: "▼";
      }

      .stage3-loop-arrow.down {
        top: -1.38rem;
      }

      .stage3-loop-arrow.up::before {
        content: "▲";
      }

      .stage3-loop-arrow.up {
        bottom: -1.38rem;
      }

      .stage3-loop-candidate {
        position: absolute;
        left: 50%;
        transform: translateX(-50%);
        font-size: 0;
      ...
      .stage3-loop-candidate::before {
        ...
        font-size: 1rem;
        line-height: 1;
        color: rgba(255, 220, 96, 0.96);
        text-shadow:
          0 0 10px rgba(255, 207, 78, 0.4),
          0 0 22px rgba(255, 188, 36, 0.22);
      }

      .stage3-loop-candidate.down {
        top: 0.95rem;
      }

      .stage3-loop-candidate.up {
        bottom: 0.95rem;
      }
